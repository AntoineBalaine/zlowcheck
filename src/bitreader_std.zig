...
pub fn BitReader(comptime endian: std.builtin.Endian) type {
    return struct {
        ...
        pub fn readBitsNoEof(self: *@This(), comptime T: type, num: u16, reader: anytype) !T {
            const b, const c = try self.readBitsTuple(T, num, reader);
        ...
        pub fn readBits(self: *@This(), comptime T: type, num: u16, out_bits: *u16, reader: anytype) !T {
            const b, const c = try self.readBitsTuple(T, num, reader);
            out_bits.* = c;
            return b;
        }
        ...
        pub fn readBitsTuple(self: *@This(), comptime T: type, num: u16, reader: anytype) !Bits(T) {
            const UT = std.meta.Int(.unsigned, @bitSizeOf(T));
            ...
            const final_byte = reader.readByte() catch |err| switch (err) {
                error.EndOfStream => return initBits(T, out, out_count),
                else => |e| return e,
            };
...
pub fn bitReader(comptime endian: std.builtin.Endian) BitReader(endian) {
    return .{};
}
...
test "api coverage" {
    ...
    try expect(1 == try bit_stream_be.readBits(u2, 1, &out_bits, reader));
    try expect(out_bits == 1);
    try expect(2 == try bit_stream_be.readBits(u5, 2, &out_bits, reader));
    try expect(out_bits == 2);
    try expect(3 == try bit_stream_be.readBits(u128, 3, &out_bits, reader));
    try expect(out_bits == 3);
    try expect(4 == try bit_stream_be.readBits(u8, 4, &out_bits, reader));
    try expect(out_bits == 4);
    try expect(5 == try bit_stream_be.readBits(u9, 5, &out_bits, reader));
    try expect(out_bits == 5);
    try expect(1 == try bit_stream_be.readBits(u1, 1, &out_bits, reader));
    try expect(out_bits == 1);

    mem_in_be.pos = 0;
    bit_stream_be.count = 0;
    try expect(0b110011010000101 == try bit_stream_be.readBits(u15, 15, &out_bits, reader));
    try expect(out_bits == 15);

    mem_in_be.pos = 0;
    bit_stream_be.count = 0;
    try expect(0b1100110100001011 == try bit_stream_be.readBits(u16, 16, &out_bits, reader));
    try expect(out_bits == 16);

    _ = try bit_stream_be.readBits(u0, 0, &out_bits, reader);

    try expect(0 == try bit_stream_be.readBits(u1, 1, &out_bits, reader));
    try expect(out_bits == 0);
    try expectError(error.EndOfStream, bit_stream_be.readBitsNoEof(u1, 1, reader));

    var mem_in_le = std.io.fixedBufferStream(&mem_le);

    reader = mem_in_le.reader();
    var bit_stream_le = bitReader(
        .little,
    );

    try expect(1 == try bit_stream_le.readBits(u2, 1, &out_bits, reader));
    try expect(out_bits == 1);
    try expect(2 == try bit_stream_le.readBits(u5, 2, &out_bits, reader));
    try expect(out_bits == 2);
    try expect(3 == try bit_stream_le.readBits(u128, 3, &out_bits, reader));
    try expect(out_bits == 3);
    try expect(4 == try bit_stream_le.readBits(u8, 4, &out_bits, reader));
    try expect(out_bits == 4);
    try expect(5 == try bit_stream_le.readBits(u9, 5, &out_bits, reader));
    try expect(out_bits == 5);
    try expect(1 == try bit_stream_le.readBits(u1, 1, &out_bits, reader));
    try expect(out_bits == 1);

    mem_in_le.pos = 0;
    bit_stream_le.count = 0;
    try expect(0b001010100011101 == try bit_stream_le.readBits(u15, 15, &out_bits, reader));
    try expect(out_bits == 15);

    mem_in_le.pos = 0;
    bit_stream_le.count = 0;
    try expect(0b1001010100011101 == try bit_stream_le.readBits(u16, 16, &out_bits, reader));
    try expect(out_bits == 16);

    _ = try bit_stream_le.readBits(u0, 0, &out_bits, reader);

    try expect(0 == try bit_stream_le.readBits(u1, 1, &out_bits, reader));
    try expect(out_bits == 0);
    try expectError(error.EndOfStream, bit_stream_le.readBitsNoEof(u1, 1, reader));
}
